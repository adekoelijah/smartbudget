import {
  AlertTriangle,
  Flag,
  RefreshCw,
  Trophy,
} from "lucide-react";

import useSmartSave from "../../../../hooks/useSmartSave";

import {
  DEFAULT_CURRENCY,
} from "../../../../constants/smartSaveConstants";

import SavingsChallengeSection from "./SavingsChallengesSection";
import CreateChallengeModal from "./ChallengeDetailsModal";

import SavingsSkeleton from "../shared/SavingsSkeleton";
import SavingsErrorState from "../shared/SavingsErrorState";

/* =========================================================
   SAFE OBJECT RESOLVER
========================================================= */

const isObject = (value) =>
  value !== null &&
  typeof value === "object" &&
  !Array.isArray(value);

/* =========================================================
   SMARTSAVE RESPONSE RESOLVER
========================================================= */

const resolveData = (data) => {
  if (!isObject(data)) {
    return {};
  }

  if (isObject(data.data)) {
    return data.data;
  }

  if (isObject(data.result)) {
    return data.result;
  }

  return data;
};

/* =========================================================
   CHALLENGE RESOLVER
========================================================= */

const resolveChallenges = (data) => {
  const challenges =
    data.challenges ??
    data.savingsChallenges;

  return Array.isArray(challenges)
    ? challenges
    : [];
};

/* =========================================================
   PAGE
========================================================= */

const SavingsChallengesPage = () => {
  const smartSave = useSmartSave();

  const {
    data,
    loading,
    error,
    refresh,
    isRefreshing,

    createChallenge,

    showCreateChallengeModal,
    setShowCreateChallengeModal,
  } = smartSave ?? {};

  /* =======================================================
     NORMALIZED DATA
  ======================================================= */

  const savingsData = resolveData(data);

  const challenges =
    resolveChallenges(savingsData);

  const currency =
    DEFAULT_CURRENCY ?? "NGN";

  const hasChallenges =
    challenges.length > 0;

  /* =======================================================
     ACTIONS
  ======================================================= */

  const handleRefresh = async () => {
    if (typeof refresh !== "function") {
      return;
    }

    try {
      await refresh();
    } catch {
      /*
       * Refresh errors remain owned by
       * useSmartSave.
       */
    }
  };

  const handleCreateChallenge = () => {
    if (
      typeof setShowCreateChallengeModal !==
      "function"
    ) {
      return;
    }

    setShowCreateChallengeModal(true);
  };

  const handleChallengeCreated = async (
    payload
  ) => {
    if (typeof createChallenge === "function") {
      await createChallenge(payload);
    }

    if (
      typeof setShowCreateChallengeModal ===
      "function"
    ) {
      setShowCreateChallengeModal(false);
    }

    if (typeof refresh === "function") {
      await refresh();
    }
  };

  const handleCloseChallengeModal = () => {
    if (
      typeof setShowCreateChallengeModal !==
      "function"
    ) {
      return;
    }

    setShowCreateChallengeModal(false);
  };

  /* =======================================================
     INITIAL LOADING
  ======================================================= */

  if (loading && !data) {
    return (
      <main
        className="
          w-full min-h-screen
          bg-slate-50
        "
        aria-busy="true"
        aria-label="Loading savings challenges"
      >
        <div
          className="
            w-full max-w-7xl
            mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8
          "
        >
          <SavingsSkeleton module="page" />
        </div>
      </main>
    );
  }

  /* =======================================================
     INITIAL ERROR
  ======================================================= */

  if (error && !data) {
    return (
      <main
        className="
          w-full min-h-screen
          bg-slate-50
        "
      >
        <div
          className="
            flex items-center
            w-full max-w-7xl min-h-screen
            mx-auto px-4 sm:px-6 lg:px-8 py-8
          "
        >
          <div
            className="
              w-full
            "
          >
            <SavingsErrorState
              error={error}
              onRetry={handleRefresh}
            />
          </div>
        </div>
      </main>
    );
  }

  /* =======================================================
     PAGE
  ======================================================= */

  return (
    <main
      className="
        w-full min-h-screen
        bg-slate-50
      "
    >
      <div
        className="
          w-full max-w-7xl
          mx-auto px-4 sm:px-6 lg:px-8 py-5 sm:py-7 lg:py-8
        "
      >
        {/* =================================================
            PAGE HEADER
        ================================================= */}

        <header
          className="
            flex flex-col sm:flex-row sm:justify-between sm:items-center
            gap-4
          "
        >
          <div
            className="
              flex items-start
              min-w-0
              gap-3
            "
          >
            <div
              className="
                flex justify-center items-center
                w-11 h-11
                bg-slate-900
                rounded-xl
                shadow-sm
                shrink-0
              "
              aria-hidden="true"
            >
              <Trophy
                size={20}
                className="
                  text-white
                "
                /
              >
            </div>

            <div
              className="
                min-w-0
              "
            >
              <p
                className="
                  font-semibold text-slate-500 text-xs uppercase tracking-wide
                "
              >
                SmartSave
              </p>

              <h1
                className="
                  mt-1
                  font-bold text-slate-900 text-xl sm:text-2xl tracking-tight
                "
              >
                Savings Challenges
              </h1>

              <p
                className="
                  max-w-2xl
                  mt-1
                  text-slate-500 text-sm leading-6
                "
              >
                Turn consistent saving into achievable
                milestones with structured challenges
                designed to keep your savings journey
                focused and engaging.
              </p>
            </div>
          </div>

          <div
            className="
              flex flex-col sm:flex-row
              gap-2
            "
          >
            <button
              type="button"
              onClick={handleRefresh}
              disabled={
                isRefreshing ||
                typeof refresh !== "function"
              }
              className="
                inline-flex justify-center items-center
                w-full sm:w-auto
                px-4 py-2.5
                font-semibold text-slate-700 text-sm
                bg-white hover:bg-slate-100
                border border-slate-200 rounded-xl focus:outline-none
                focus:ring-2 focus:ring-slate-300
                disabled:opacity-50 shadow-sm transition
                disabled:cursor-not-allowed
                gap-2
              "
              aria-label="Refresh savings challenges"
            >
              <RefreshCw
                size={15}
                className={
                  isRefreshing
                    ? "animate-spin"
                    : ""
                }
                aria-hidden="true"
              />

              {isRefreshing
                ? "Updating..."
                : "Refresh"}
            </button>

            <button
              type="button"
              onClick={handleCreateChallenge}
              disabled={
                typeof setShowCreateChallengeModal !==
                "function"
              }
              className="
                inline-flex justify-center items-center
                w-full sm:w-auto
                px-4 py-2.5
                font-semibold text-white text-sm
                bg-slate-900 hover:bg-slate-800
                border border-slate-900 rounded-xl focus:outline-none
                focus:ring-2 focus:ring-slate-300
                disabled:opacity-50 shadow-sm transition
                disabled:cursor-not-allowed
                gap-2
              "
            >
              <Flag
                size={15}
                aria-hidden="true"
              />

              New challenge
            </button>
          </div>
        </header>

        {/* =================================================
            REFRESH STATUS
        ================================================= */}

        {isRefreshing && (
          <div
            className="
              flex items-center
              mt-4 px-4 py-2.5
              font-medium text-slate-500 text-xs
              bg-white
              border border-slate-200 rounded-xl
              shadow-sm
              gap-2
            "
            role="status"
            aria-live="polite"
          >
            <RefreshCw
              size={13}
              className="
                animate-spin
              "
              aria-hidden="true"
            /
            >

            Updating your savings challenges...
          </div>
        )}

        {/* =================================================
            PARTIAL ERROR
        ================================================= */}

        {error && data && (
          <div
            className="
              flex flex-col sm:flex-row sm:justify-between sm:items-center
              mt-4 p-4
              bg-amber-50
              border border-amber-200 rounded-xl
              gap-3
            "
            role="alert"
          >
            <div
              className="
                flex items-start
                min-w-0
                gap-3
              "
            >
              <AlertTriangle
                size={17}
                className="
                  mt-0.5
                  text-amber-600
                  shrink-0
                "
                aria-hidden="true"
              /
              >

              <div
                className="
                  min-w-0
                "
              >
                <p
                  className="
                    font-semibold text-amber-900 text-sm
                  "
                >
                  Challenge data may be out of date.
                </p>

                <p
                  className="
                    mt-0.5
                    text-amber-700 text-xs leading-5
                  "
                >
                  Your previously loaded challenges
                  remain available.
                </p>
              </div>
            </div>

            <button
              type="button"
              onClick={handleRefresh}
              disabled={isRefreshing}
              className="
                inline-flex justify-center items-center
                w-full sm:w-auto
                px-3 py-2
                font-semibold text-amber-800 text-xs
                bg-white hover:bg-amber-100
                border border-amber-200 rounded-lg
                disabled:opacity-50 transition
                disabled:cursor-not-allowed
                gap-2 shrink-0
              "
            >
              <RefreshCw
                size={13}
                className={
                  isRefreshing
                    ? "animate-spin"
                    : ""
                }
                aria-hidden="true"
              />

              Retry
            </button>
          </div>
        )}

        {/* =================================================
            CHALLENGE SUMMARY
        ================================================= */}

        <section
          className="
            grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3
            mt-6 sm:mt-8
            gap-4
          "
          aria-label="Savings challenge summary"
        >
          <div
            className="
              p-5
              bg-white
              border border-slate-200 rounded-2xl
              shadow-sm
            "
          >
            <div
              className="
                flex justify-between items-center
                gap-3
              "
            >
              <p
                className="
                  font-medium text-slate-500 text-xs uppercase tracking-wide
                "
              >
                Challenges
              </p>

              <div
                className="
                  flex justify-center items-center
                  w-8 h-8
                  bg-slate-100
                  rounded-lg
                "
                aria-hidden="true"
              >
                <Trophy
                  size={15}
                  className="
                    text-slate-700
                  "
                  /
                >
              </div>
            </div>

            <p
              className="
                mt-3
                font-bold text-slate-900 text-2xl
              "
            >
              {challenges.length}
            </p>

            <p
              className="
                mt-1
                text-slate-500 text-xs leading-5
              "
            >
              Savings challenges currently available
              in your SmartSave workspace.
            </p>
          </div>

          <div
            className="
              p-5
              bg-white
              border border-slate-200 rounded-2xl
              shadow-sm
            "
          >
            <p
              className="
                font-medium text-slate-500 text-xs uppercase tracking-wide
              "
            >
              Challenge mode
            </p>

            <p
              className="
                mt-2
                font-bold text-slate-900 text-2xl
              "
            >
              Goal focused
            </p>

            <p
              className="
                mt-1
                text-slate-500 text-xs leading-5
              "
            >
              Build consistency through structured
              saving targets and milestones.
            </p>
          </div>

          <div
            className="
              p-5
              bg-white
              border border-slate-200 rounded-2xl
              shadow-sm
            "
          >
            <p
              className="
                font-medium text-slate-500 text-xs uppercase tracking-wide
              "
            >
              Currency
            </p>

            <p
              className="
                mt-2
                font-bold text-slate-900 text-2xl
              "
            >
              {currency}
            </p>

            <p
              className="
                mt-1
                text-slate-500 text-xs leading-5
              "
            >
              Your SmartSave financial display
              currency.
            </p>
          </div>
        </section>

        {/* =================================================
            EMPTY STATE
        ================================================= */}

        {!hasChallenges && (
          <section
            className="
              mt-6 p-6 sm:p-8
              bg-white
              border border-slate-200 rounded-2xl
              shadow-sm
            "
            aria-labelledby="challenge-empty-title"
          >
            <div
              className="
                flex flex-col items-center
                max-w-xl
                mx-auto
                text-center
              "
            >
              <div
                className="
                  flex justify-center items-center
                  w-14 h-14
                  bg-slate-100
                  rounded-2xl
                "
                aria-hidden="true"
              >
                <Flag
                  size={25}
                  className="
                    text-slate-700
                  "
                  /
                >
              </div>

              <h2
                id="challenge-empty-title"
                className="
                  mt-5
                  font-bold text-slate-900 text-lg sm:text-xl tracking-tight
                "
              >
                Start your first savings challenge
              </h2>

              <p
                className="
                  max-w-lg
                  mt-2
                  text-slate-500 text-sm leading-6
                "
              >
                Create a structured savings challenge
                to turn a target into a series of
                achievable actions. SmartSave will keep
                your progress connected to the rest of
                your savings journey.
              </p>

              <button
                type="button"
                onClick={handleCreateChallenge}
                disabled={
                  typeof setShowCreateChallengeModal !==
                  "function"
                }
                className="
                  inline-flex justify-center items-center
                  mt-6 px-5 py-2.5
                  font-semibold text-white text-sm
                  bg-slate-900 hover:bg-slate-800
                  rounded-xl focus:outline-none
                  focus:ring-2 focus:ring-slate-300
                  disabled:opacity-50 shadow-sm transition
                  disabled:cursor-not-allowed
                  gap-2
                "
              >
                <Flag
                  size={15}
                  aria-hidden="true"
                />

                Create challenge
              </button>
            </div>
          </section>
        )}

        {/* =================================================
            CHALLENGES
        ================================================= */}

        {hasChallenges && (
          <section
            className="
              mt-6 sm:mt-8
            "
            aria-labelledby="savings-challenges-heading"
          >
            <div
              className="
                flex flex-col sm:flex-row sm:justify-between sm:items-end
                mb-4
                gap-3
              "
            >
              <div>
                <h2
                  id="savings-challenges-heading"
                  className="
                    font-bold text-slate-900 text-lg
                  "
                >
                  Your savings challenges
                </h2>

                <p
                  className="
                    mt-1
                    text-slate-500 text-sm
                  "
                >
                  Stay consistent, track progress, and
                  turn each challenge into measurable
                  savings momentum.
                </p>
              </div>

              <span
                className="
                  inline-flex items-center
                  w-fit
                  px-2.5 py-1
                  font-semibold text-slate-600 text-xs
                  bg-slate-100
                  rounded-full
                "
              >
                {challenges.length}{" "}
                {challenges.length === 1
                  ? "challenge"
                  : "challenges"}
              </span>
            </div>

            <SavingsChallengeSection
              challenges={challenges}
              onCreate={handleCreateChallenge}
            />
          </section>
        )}

        {/* =================================================
            FOOTER
        ================================================= */}

        <footer
          className="
            mt-8 sm:mt-10 pt-5
            border-slate-200 border-t
          "
        >
          <div
            className="
              flex flex-col sm:flex-row sm:justify-between sm:items-center
              text-slate-400 text-xs
              gap-2
            "
          >
            <p>
              SmartSave turns intentional saving into
              measurable progress.
            </p>

            <p
              className="
                font-medium
              "
            >
              Currency: {currency}
            </p>
          </div>
        </footer>
      </div>

      {/* =================================================
          CREATE CHALLENGE MODAL
      ================================================= */}

      {showCreateChallengeModal && (
        <CreateChallengeModal
          open={showCreateChallengeModal}
          onClose={handleCloseChallengeModal}
          onSubmit={handleChallengeCreated}
        />
      )}
    </main>
  );
};

export default SavingsChallengesPage;